...
def string_to_bit_array(text):  #Convert a string into a list of bits
    ...
    for char in text:
        ...
        array.extend([int(x)
                      for x in list(binval)])  #Add the bits to the final list
    return array


def bit_array_to_string(array):  #Recreate the string from the bit array
    res = ''.join([
        chr(int(y, 2)) for y in
...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        if padding and action == ENCRYPT:
            self.addPadding()
                ...
                self.text
        ) % 8 != 0:  #If not padding specified data size must be multiple of 8 bytes
            raise "Data size should be multiple of 8"

        self.generatekeys()  #Generate all the keys
        text_blocks = nsplit(
            self.text, 8)  #Split the text in blocks of 8 bytes so 64 bits
        print(text_blocks)
        result = list()
    ...
    def substitute(self, d_e):  #Substitute bytes using SBOX
        ...
        for i in range(len(subblocks)):  #For all the sublists
            ...
            result += [int(x)
                       for x in bin]  #And append it to the resulting list
        return result
    ...
    def generatekeys(self):  #Algorithm that generates all the keys
        ...
        for i in range(16):  #Apply the 16 rounds
            ...
            tmp = g + d  #Merge them
            self.keys.append(self.permut(
                tmp, CP_2))  #Apply the permut to get the Ki
